...
- [ ] **Shared persistence and identity** — wallets/wagers are still per-browser seeded mock state; real league use needs a backend (or hosted store) plus member identity so everyone sees the same book. Needs a hosting/identity decision before building.
