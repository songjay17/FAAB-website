...
- [ ] **[IMPORTANT] Collusion-resistant odds calculation** — odds must be derived from each team's full-roster optimal (best-possible) lineup, not whatever lineup is actually set, so an owner can't tank their started lineup right before betting opens to manufacture bad odds on themselves for a colluding bettor to profit from. Needs: a real optimal-lineup selection algorithm (QB/RB/RB/WR/WR/TE/FLEX/K/DEF, FLEX eligible for RB/WR/TE, maximize total `projectedPoints`) feeding `projectionToMoneylines`, plus freezing computed odds once a market opens for the week so no later roster change can move an already-posted line.
