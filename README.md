...
- [ ] **Identity & enforcement (PR 2 of the shared-persistence plan)** — the book is shared now, but the actor is still client-asserted (`memberId` in the request body) and commissioner powers are unenforced. Next: claim-your-member + PIN login, session cookie, every mutation route deriving the actor server-side, commissioner checks, and the commissioner audit UI reading the server `audit_log` table. See [docs/shared-persistence-plan.md](docs/shared-persistence-plan.md).
