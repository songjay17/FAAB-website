...
- [ ] **Offseason/season-complete UX** — "week 17, all matchups final, 0 FAAB" is the app's real state for months every year. Detect `league.status === "complete"` and show a proper season-over experience (final leaderboard, betting closed) instead of a dead betting UI.
- [ ] **2026 league rollover** — Sleeper mints a new league id each season (2026 preseason is underway). Walk the new league's `previous_league_id` chain or make the id a first-class config step so the app follows the league year to year.
- [ ] **Fix `currentWeek` source** — `league.settings.last_scored_leg` (`src/lib/sleeper/mappers.ts`) pins to 17 forever on a completed season and 0 on a fresh one; combine `/v1/state/nfl` (`display_week`) with league status instead.
